...
//展示标签.  ~
function showLabelClassify(lables, classifys) {  //"lables", "classifys"
    var labelHtml = "";
    var classifyHtml = "";
    var lcs = getLabelClassify();//得到所有的标签,分类.
    jQuery.each(lcs.labels, function (idx, obj) {
        labelHtml += "<input type='checkbox' value='" + obj.t_id + "' data-id='" + obj.t_id + "' title='" + obj.t_name + "' lay-skin='primary'>";
    });
    jQuery.each(lcs.classifys, function (idx, obj) {
        classifyHtml += "<input type='checkbox' value='" + obj.t_id + "' data-id='" + obj.t_id + "' title='" + obj.t_name + "' lay-skin='primary'>";
    });
...
function getLabelClassify() {
    ...
    if (v == null || v === "" || v === undefined) {  //如果session中,没有这些数据,去取
        jQuery.ajax({
            ...
            success: function (result, status, xhr) {
                layer.closeAll('loading');
                sessionStorage.setItem("LabelClassify", JSON.stringify(result));
            }
        });
    } //如果session中,有这些数据,直接返回.
