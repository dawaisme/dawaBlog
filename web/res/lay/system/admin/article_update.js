...
layui.use(['layer', 'element', 'form', 'upload'], function () {
    ...
    //修改按钮的事件监听.  //暂时先不用, 获取不到ajax提供的数据,
    /*function submit(article_data) {
        alert(article_data);
        $.ajax({
            url: IP + '/admin/admin.do?cmd=updateArticle',
            type: 'PUT',
            data: article_data,
    ...
    //初始化标签.和分类
    function initData() {
        var id = getUrlParam("a_id");
        showLabelClassify("lables", "classifys"); //展示标签
        form.render('checkbox'); //展示OMG
        showData(id);//回显数据
    }


    //回显数据
    function showData(id) { //展示文章数据.
        $.ajax({
            url: IP + '/admin/admin.do?cmd=getArticleById&id=' + id,
            type: 'post',
            dataType: 'json',
            success: function (result, status, xhr) {
                console.log(result);
                ...
                layer.closeAll('loading');
                $("input[name='id']").val(result.data.a_id);
                $("input[name='title']").val(result.data.a_title);
                $("textarea[name='summary']").val(result.data.a_brief);
                $('.layui-upload-img').attr('src', result.data.a_image);
                $('#layui-upload-img').val(result.data.a_image);

                $.each(result.ids, function (idx, obj) {
                    $("#lables input[type='checkbox'][value='" + obj.t_id + "']").attr("checked", "checked")
                });
                $.each(result.ids, function (idx, obj) {
                    $("#classifys input[type='checkbox'][value='" + obj.t_id + "']").attr("checked", "checked")
                });
                if (result.data.is_top === "置顶") {
                    $("input[name='is_top']").attr("checked", "checked")
                }
                if (result.data.is_view === "可见") {
                    $("input[name='is_view']").attr("checked", "checked")
                }
                form.render('checkbox');
                editor.txt.html(result.data.a_content);
            }
        });
    }
});